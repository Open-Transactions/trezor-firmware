...
from trezor.ui.text import BR, Text, render_text
...
async def show_checklist_set_shares(ctx):
    cl = _shamir_checklist()
    cl.select(0)
    cl.process()
    return await confirm(
        ctx, cl, code=ButtonRequestType.ResetDevice, cancel=None, confirm="Set shares"
    )
...
async def show_checklist_set_treshold(ctx, num_of_shares):
    ...
    cl.process()
    return await confirm(
        ctx, cl, code=ButtonRequestType.ResetDevice, cancel=None, confirm="Set treshold"
    )
...
def _shamir_checklist():
    cl = Checklist("Backup checklist", ui.ICON_RESET)
    cl.add("Set number of shares")
    cl.add("Set the treshold")
    cl.add(("Write down and check", "all seed shares"))
    return cl
...
class Checklist(ui.Control):
    ...
    def on_render(self):
        if self.repaint:
            ...
            render_text(
                self.words,
                new_lines=False,  # we are adding BR manually
...
async def confirm_word(ctx, share_index, numbered_share):
    ...
    numbered_choices = numbered_share[: WordSelect.NUM_OF_CHOICES]
    ...
    select = WordSelect(choices, share_index, checked_index)
